...
// Admin credentials - in a production app, these should be environment variables
const ADMIN_EMAIL = "admin123@gmail.com";
const ADMIN_PASSWORD = "111111"; // Hardcoded for demo purposes only
...
export function AdminProvider({ children }) {
  ...
  function adminLogin(email, password) {
    // Validate that it's the admin email
    if (email.toLowerCase() !== ADMIN_EMAIL.toLowerCase()) {
      return Promise.reject(new Error('Invalid admin credentials'));
    }
    
    // Direct bypass for hardcoded admin credentials - no Firebase auth required
    if (email.toLowerCase() === ADMIN_EMAIL.toLowerCase() && password === ADMIN_PASSWORD) {
      // Create a mock user object without going through Firebase Auth
      const mockAdminUser = {
        uid: 'admin-user-id',
        email: ADMIN_EMAIL,
      ...
      // Return a resolved promise with the mock user
      return Promise.resolve(mockAdminUser);
    }
    
    // For other credentials, perform the normal check
    return signInWithEmailAndPassword(auth, email, password)
      .then(userCredential => {
        return getDoc(doc(db, 'admins', userCredential.user.uid))
          .then(adminDoc => {
            if (!adminDoc.exists()) {
              // If the user is not in the admins collection, log them out
              signOut(auth);
